    ...
    <form action="{{ route('admin.layanan.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="mt-2"><textarea name="deskripsi" id="deskripsi" rows="4" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300" required>{{ old('deskripsi') }}</textarea></div>
            </div>
            <div>
                <label for="harga" class="block text-sm font-medium leading-6 text-gray-900">Harga</label>
                <div class="mt-2"><input type="number" name="harga" id="harga" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300" required value="{{ old('harga') }}"></div>
            </div>
            {{-- INPUT UNTUK GAMBAR --}}
            <div>
