...
@section('content')
    {{-- Section 1: Header Cerita Kami dengan 1 Gambar Utama --}}
    <div class="bg-white py-16 sm:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 items-center gap-x-16 gap-y-16 lg:grid-cols-2">
                {{-- Kolom Kiri: Satu Gambar Utama (KEMBALI KE VERSI AWAL) --}}
                <div class="aspect-h-2 aspect-w-3 overflow-hidden rounded-lg bg-stone-100">
                    <img src="{{ asset('images/gambar1.jpg') }}" alt="Tampak depan H2O Barbershop" class="h-full w-full object-cover object-center">
                </div>
    ...
    {{-- Section 2: Temui Tim Profesional Kami --}}
        ...
        <div class="mx-auto max-w-7xl px-6 text-center lg:px-8">
            ...
            <ul role="list" class="mx-auto mt-20 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-3">
                @forelse ($capsters as $capster)
                    <li>
                        @if ($capster->foto)
                            <img class="mx-auto h-56 w-56 rounded-full object-cover" src="{{ asset('storage/' . $capster->foto) }}" alt="Foto {{ $capster->nama }}">
                        @else
                            <img class="mx-auto h-56 w-56 rounded-full object-cover" src="https://ui-avatars.com/api/?name={{ urlencode($capster->nama) }}&size=224&color=FFFFFF&background=292524" alt="Avatar {{ $capster->nama }}">
                        @endif
                        <h3 class="mt-6 text-base font-semibold leading-7 tracking-tight text-stone-900">{{ $capster->nama }}</h3>
                        <p class="text-sm leading-6 text-stone-600">{{ $capster->deskripsi }}</p>
                    </li>
                @empty
                    <li class="col-span-3">
    ...
    {{-- Section 3: Ajakan Booking (Call to Action) --}}
    <div class="bg-stone-800">
        <div class="mx-auto max-w-4xl px-6 py-16 text-center sm:px-6 sm:py-20 lg:px-8">
            ...
            <a href="{{ route('booking.create') }}" class="mt-8 inline-flex w-full items-center justify-center rounded-md border border-transparent bg-amber-500 px-5 py-3 text-base font-medium text-stone-900 hover:bg-amber-400 sm:w-auto">Booking Jadwalmu Sekarang</a>
        </div>
    </div>
@endsection
