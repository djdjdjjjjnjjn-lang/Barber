...
@section('content')
{{-- Section 1: Judul Utama & Daftar Layanan --}}
        ...
        <div class="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="flex flex-col rounded-lg bg-white shadow-lg overflow-hidden transition-transform duration-300 hover:-translate-y-2">
                <div class="flex-shrink-0">
                    @if ($layanan->gambar)
                        <img class="h-56 w-full object-cover" src="{{ asset('storage/' . $layanan->gambar) }}" alt="Gambar {{ $layanan->nama }}">
                    @else
                ...
                <div class="flex flex-1 flex-col justify-between p-6">
                    <div class="flex-1">
                        ...
                        <p class="mt-3 text-base text-stone-600">{{ $layanan->deskripsi }}</p>
                    </div>
                    <div class="mt-6 flex items-center justify-between">
                        <p class="text-lg font-medium text-stone-800">
                            Rp {{ number_format($layanan->harga, 0, ',', '.') }}
                        </p>
                        <a href="{{ route('booking.create') }}" class="rounded-md bg-stone-800 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-stone-700 transition-colors">Booking</a>
                    </div>
                </div>
            </div>
            @empty
...
{{-- Section 3: Ajakan Booking Terakhir (BAGIAN BARU) --}}
<div class="bg-stone-800">
    <div class="mx-auto max-w-4xl px-6 py-16 text-center sm:px-6 sm:py-20 lg:px-8">
        ...
        <a href="{{ route('booking.create') }}" class="mt-8 inline-flex w-full items-center justify-center rounded-md border border-transparent bg-white px-5 py-3 text-base font-medium text-stone-900 hover:bg-stone-50 sm:w-auto">Booking Sekarang</a>
    </div>
</div>
@endsection
