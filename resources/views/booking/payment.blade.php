        ...
        <div class="mt-10 bg-gray-50 p-8 rounded-lg shadow-md">
            ...
            <dl class="mt-4 space-y-4">
                <div class="flex justify-between">
                    ...
                    <dd class="text-sm font-medium text-gray-900">{{ $booking->nama_pelanggan }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-600">Nomor HP</dt>
                    <dd class="text-sm font-medium text-gray-900">{{ $booking->no_hp_pelanggan }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-600">Layanan</dt>
                    <dd class="text-sm font-medium text-gray-900">{{ $booking->layanan->nama }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-600">Capster</dt>
                    <dd class="text-sm font-medium text-gray-900">{{ $booking->capster->nama }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-600">Jadwal</dt>
                    <dd class="text-sm font-medium text-gray-900">{{ \Carbon\Carbon::parse($booking->tanggal_booking)->format('d F Y') }}, Pukul {{ \Carbon\Carbon::parse($booking->jam_booking)->format('H:i') }}</dd>
                </div>
                <div class="flex justify-between border-t border-gray-200 pt-4">
                    <dt class="text-base font-semibold text-gray-900">Total Pembayaran</dt>
                    <dd class="text-base font-semibold text-gray-900">Rp {{ number_format($booking->total_harga) }}</dd>
                </div>
            </dl>
            ...
            <div class="mt-8 text-center">
                ...
                <img src="https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=ContohQRIS-Barbershop-Booking-{{$booking->id}}" alt="QRIS Pembayaran" class="mx-auto mt-4 border rounded-lg">
                ...
                <a href="{{ route('home') }}" class="mt-6 inline-block w-full rounded-md bg-green-600 px-3.5 py-2.5 text-center text-sm font-semibold text-white shadow-sm hover:bg-green-500">
